#include <utm50_uavis2/RiskAwareBicriteriaDijkstraInstance.hh>
#include <utm50_uavis2/RiskAwareNeighbours.hh>
...
std::vector<Path> BicriteriaDijkstraInstance::computeParetoApxPaths() {
    ...
    duration = std::chrono::duration_cast<std::chrono::seconds>(time3 - time2);
    std::cout << "Time elapsed is: " << duration.count() << "s\n";
    ...
    if (beta < - 0.0000001) {
        ...
        duration = std::chrono::duration_cast<std::chrono::seconds>(time4 - time3);
        std::cout << "Time elapsed is: " << duration.count() << "s\n";
...
Path BicriteriaDijkstraInstance::runWithAlpha(double alpha) {
    ...
    int loop_count = 0; // to compare with AStar
    while (!pq.empty()) {
        ...
        Neighbours neigbours = this->risk_map.neighboursWithin(current_node, this->search_limit, this->offset);
    ...
    std::cout << "Dijkstra loop_count: " << loop_count << "\n";
