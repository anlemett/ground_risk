...
bool compareCoordXFirst(Coord p1, Coord p2)
{
    if (p1.x == p2.x) {
        return p1.y < p2.y;
    }
...
bool compareCoordYFirst(Coord p1, Coord p2)
{
    if (p1.y == p2.y) {
        return p1.x < p2.x;
    }
...
ParallelogramPixelsIter::ParallelogramPixelsIter(Coord l, Coord r, Coord b, Coord t, Coord current_point) {
    ...
    this->current_point.x = floor(l.x);
...
Coord& ParallelogramPixelsIter::operator*() const { 
    return *current_point_ptr; 
}

Coord* ParallelogramPixelsIter::operator->() {
    return current_point_ptr;
}
...
int ParallelogramPixelsIter::getMinY(Coord p0, Coord p1, float x) { 
...
int ParallelogramPixelsIter::getMaxY(Coord p0, Coord p1, float x) { 
    // Thanks to Markus Jarderot from https://stackoverflow.com/questions/5610616/finding-all-pixels-at-least-partially-within-an-arbitrarily-oriented-rectangle
    // for inspiration.
...
ParallelogramPixels::ParallelogramPixels(Side origin_side, Side destination_side)
{
    ...
    std::vector<Coord> points = {origin_side.first, origin_side.second, destination_side.first, destination_side.second};
    
    // constructor method, Deep copy 
    std::vector<Coord> l_mid1_mid2_r(points);
    
    sort(l_mid1_mid2_r.begin(), l_mid1_mid2_r.end(), compareCoordXFirst);
    
    Coord l = l_mid1_mid2_r.at(0), mid1 = l_mid1_mid2_r.at(1), mid2 = l_mid1_mid2_r.at(2), r = l_mid1_mid2_r.at(3);
        
    std::vector<Coord> mid_points = {mid1, mid2};
    
    std::vector<Coord> b_t(mid_points);
    
    std::sort(b_t.begin(), b_t.end(), compareCoordYFirst);
    
    Coord b = b_t.at(0), t = b_t.at(1);
}
...
ParallelogramPixelsIter ParallelogramPixels::begin() { 
    ...
    Coord current_point = Coord(l.x, l.y);
...
ParallelogramPixelsIter ParallelogramPixels::end() {//used to determine when the boundary has been reached: it should not be accessed directly
    ...
    Coord current_point = Coord(r.x, r.y);
    return ParallelogramPixelsIter(l, r, b, t, current_point);
}
